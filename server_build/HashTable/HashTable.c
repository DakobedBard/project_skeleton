...
void FreeHashTable(HashTable table,
  ...
  AssertionWrapper(table != NULL);  // be defensive
  ...
  for (i = 0; i < table->num_buckets; i++) {
    ...
    while (NumElementsInLinkedList(bl) > 0) {
      AssertionWrapper(PopLinkedList(bl, (void **) &nextKV));
...
HTIter HashTableMakeIterator(HashTable table) {
  ...
  AssertionWrapper(i < table->num_buckets);  // make sure we found it.
...
int HTIteratorDelete(HTIter iter, HTKeyValue *keyvalue) {
  ...
  AssertionWrapper(res == 1);
  AssertionWrapper(kv.key == keyvalue->key);
  AssertionWrapper(kv.value == keyvalue->value);
...
static void ResizeHashtable(HashTable ht) {
  ...
  while (!HTIteratorPastEnd(it)) {
    ...
    AssertionWrapper(HTIteratorGet(it, &item) == 1);
